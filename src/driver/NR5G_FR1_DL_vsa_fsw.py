...
class VSA_driver(VSADriver):
    ...
    def vsa_configure(self) -> None:
        ...
        self.VSA.write(':CONF:NR5G:LDIR DL')                            # Link Direction
        self.VSA.write(':CONF:NR5G:DL:CC1:DFR MIDD')                    # Band
        self.VSA.write(':CONF:NR5G:DL:CC1:FRAM1:BWP0:SLOT0:CRSC 0')     # CORESET 0
        self.VSA.write(':CONF:NR5G:DL:CC1:FRAM1:BWP0:SLOT0:ALL0:SOFF 0') # Symbol Offset
        self.VSA.write(':CONF:NR5G:DL:CC1:FRAM1:BWP0:SLOT0:ALL0:SCO 14') # Symbol Offset
        self.VSA.write(f':CONF:NR5G:DL:CC1:BW BW{self.bw}')             # BW
        self.VSA.write(f':CONF:NR5G:DL:CC1:FRAM1:BWP0:SSP SS{self.scs}') # SCS
        self.VSA.write(':CONF:NR5G:DL:CC1:FRAM1:BWP0:CSL 1')            # User Config Slot
        self.VSA.write(f':CONF:NR5G:DL:CC1:FRAM1:BWP0:RBC {self.rb}')   # BWP RB
        self.VSA.write(f':CONF:NR5G:DL:CC1:FRAM1:BWP0:RBOF {self.rbo}') # BWP RB Offset
        self.VSA.write(f':CONF:NR5G:DL:CC1:FRAM1:BWP0:SLOT0:ALL0:RBC {self.rb}')
        self.VSA.write(f':CONF:NR5G:DL:CC1:FRAM1:BWP0:SLOT0:ALL0:RBOF {self.rbo}')
        self.VSA.write(':CONF:NR5G:DL:CC1:FRAM1:BWP0:SLOT0:ALL0:MOD QAM64')   # Q1k; QAM64; QAM256

        # Results Table Data Removal
        self.VSA.write(':INIT:CONT OFF')                        #
        self.VSA.write(':DISP:WIND2:TABL:ITEM DSQP,OFF')        #
        self.VSA.write(':DISP:WIND2:TABL:ITEM DS1K,OFF')        #
        self.VSA.write(':DISP:WIND2:TABL:ITEM DSST,OFF')        #
        self.VSA.write(':DISP:WIND2:TABL:ITEM DSSF,OFF')        #
        self.VSA.write(':DISP:WIND2:TABL:ITEM DSTS,OFF')        #
    ...
    def vsa_get_waveform_info(self) -> str:
        ...
        if ldir == 'UL':
            tpre = self.VSA.query(f':CONF:NR5G:DL:CC1:TPR?')            # Trans Precoding State
        else:
            tpre = 'Off'
        ...
        outStr = f'{freq}GHz_{frng}_{ldir}_{chbw}_{bscs}_{bwrb}_{cmod}_TP{tpre}_PhaseComp{phas}'
