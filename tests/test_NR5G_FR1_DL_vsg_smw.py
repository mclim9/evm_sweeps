...
class TestNR5G_FR1_VSG_SMW(unittest.TestCase):
    def setUp(self):
        # Patch BenchConfig to avoid real socket connections during tests
        self.bench_patcher = patch('driver.NR5G_FR1_DL_vsg_smw.BenchConfig')
    ...
    def test_init_sets_up_vsg(self):
        """Test that __init__ starts VSG connection and sets default parameters."""
        self.mock_bench_instance.VSG_start.assert_called_once()
        self.assertEqual(self.driver.VSG, self.mock_vsg)
        self.assertEqual(self.driver.freq, 6e9)
        self.assertEqual(self.driver.scs, 30)
        self.assertEqual(self.driver.rb, 273)
        self.assertEqual(self.driver.rbo, 0)
        self.assertEqual(self.driver.bw, 100)
        self.assertEqual(self.driver.pwr, -10)
    ...
    def test_vsg_configure_sends_scpi(self):
        ...
        self.mock_vsg.write.assert_any_call(':SOUR1:BB:NR5G:LINK DOWN')
        self.mock_vsg.write.assert_any_call(f':SOUR1:BB:NR5G:QCKS:GEN:CBW BW{self.driver.bw}')
        self.mock_vsg.write.assert_any_call(f':SOUR1:BB:NR5G:QCKS:GEN:SCSP SCS{self.driver.scs}')
        self.mock_vsg.write.assert_any_call(':SOUR1:BB:NR5G:QCKS:APPL')
        self.mock_vsg.write.assert_any_call(':OUTP1:STAT 1')
        self.mock_vsg.query.assert_any_call(':SOUR1:CORR:OPT:EVM 1;*OPC?')
        self.mock_vsg.query.assert_any_call('*OPC?') # Last OPC query

        # Verify vsg_set_power was called internally
        self.mock_vsg.write.assert_any_call(f':SOUR1:POW:POW {self.driver.pwr}')
