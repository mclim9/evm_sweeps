...
from driver.OFDM_vsg_smw import VSG_driver

class Test_OFDM_VSG_SMW(unittest.TestCase):
    def setUp(self):
        # Patch BenchConfig to avoid real socket connections during tests
        self.bench_patcher = patch('driver.OFDM_vsg_smw.BenchConfig')
    ...
    def test_vsg_configure_sends_scpi(self):
        ...
        self.mock_vsg.write.assert_any_call(':SOUR1:BB:OFDM:STAT 1')
    ...
    def test_vsg_get_extra(self):
        ...
        self.assertEqual(self.driver.vsg_get_extra(), 'SMW-OFDM')
    ...
    def test_vsg_save_state(self):
        ...
        self.mock_vsg.query.side_effect = ["OFDM_XML_File", "Second"]
        self.mock_vsg.s.getpeername.return_value = ("192.168.1.51", 5025)

        with patch('driver.OFDM_vsg_smw.os.system') as mock_sys:
            self.driver.vsg_save_state()

            expected_wavename = 'OFDM_XML_File'
